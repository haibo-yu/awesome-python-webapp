...
import time           #time 模块提供了一些处理日期和一天内时间的函数
import uuid           #uuid是一种唯一标识，在许多领域作为标识用途。python的uuid模块就是用来生成它的。
import functools      #functools是函数式编程模块库，可提供偏函数等
import threading      #threading提供了一个比thread模块更高层的API来提供线程的并发性。这些线程并发运行并共享内存。
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    ...
    for k, v in defaults.iteritems():
        params[k] = kw.pop(k, v)    #向parms中写入默认值或kw传入值
    params.update(kw)  #params与kw传入值合并
